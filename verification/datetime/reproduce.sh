...
# reachability. Run from this directory. Exits 0 only if every leg matches.
#
#   ./reproduce.sh            all three legs
#   ./reproduce.sh esbmc      one leg: esbmc | sanitizer | reachability
...
leg_esbmc() {
  echo "[1/3] ESBMC -- find the defects symbolically"
...
leg_sanitizer() {
  echo "[2/3] UBSan -- confirm on the real 1.11.869 source"
  mkdir -p results
...
leg_reachability() {
  echo "[3/3] Reachability -- untrusted input reaches both, at $UPSTREAM_COMMIT"
...
case "${1:-all}" in
  ...
  all) leg_esbmc; leg_sanitizer; leg_reachability ;;
  *) echo "usage: $0 [all|esbmc|sanitizer|reachability]"; exit 2 ;;
esac
